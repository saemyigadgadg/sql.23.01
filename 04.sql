...
select to_char(sysdate, 'yyyy-mm-dd') --내가 지정할 수 있다.
from dual;
...
select last_name, to_char(hire_date,'yyyy.mm.dd') hire_date, 
        to_char(next_day(hire_date + TO_YMINTERVAL('00-03'), 2),'yyyy.mm.dd') 
        review_date
from employees;
...
select last_name, hire_date,
    case when hire_date <= to_date('2005/12/31','yyyy.mm.dd') then 100
         when hire_date >= to_date('2006/01/01','yyyy.mm.dd') then 10 
    end "gift card"
from employees;

-- 선생님 답
select last_name, hire_date,
    case when hire_date <= to_date('2005/12/31','yyyy.mm.dd') then '100만원'
         else '10만원' end gift
from employees
order by  gift, hire_date;

--이전 : 그 때를 포함하는 전 시기 혹은 앞 순서
--이후 : 그 때를 포함하는 후 시기 혹은 뒷 순서
-- 전 : 그 때를 포함하지 않는 앞 시기 혹은 앞 순서
-- 후 : 그 때를 포함하지 않는 뒤 시기 혹은 귓 순서
